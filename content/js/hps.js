(function (root, factory) {
  ...
  var HPS = function(wrp, options){
    ...
    var setupStyles = function() {
      ...
      setContentWidth = document.getElementsByClassName('productHolder');
      
      if(deviceWidth > 1365){
        contentHeight = deviceHeight - 240;
        let size = setContentWidth.length;

        for(let i =0; i< size; i++){
          let contents = setContentWidth[i];
          contents.style.width= 27+'vw';
          contents.style.height= contentHeight+'px';
          contentWidth = contents.offsetWidth+25;
          //alert(contentWidth)
        }
      }
      if(deviceWidth < 1365){
        ...
        for(let i =0; i< size; i++){
          let contents = setContentWidth[i];
          contents.style.width= 300+"px";
          contentWidth = contents.offsetWidth+25;
          contents.style.height= 70+'%';
        }
      ...
      applyStyle(self.wrapper, {
        width: contentWidth * self.sections.length
      });
    ...
    var setupControls = function() {
      ...
      //temp
      controlsNode.style.cssText = 'position:fixed;';
      var arrowLeft = document.createElement('button');
      arrowLeft.classList.add(self.options.controls.prevButtonClass);
      //temp
      arrowLeft.style.cssText = 'font-size: 16px; float:left; margin-right:30px';
      ...
      //temp
      arrowRight.style.cssText = 'font-size: 16px';
    ...
    var numListeners,
      ...
      if (!self.options.scrollCallback) {
        if (e.type == 'click') {
          ...
          targetX += -event.deltaX || event.deltaY;
          targetX = Math.max( ((contentWidth * self.sections.length) - contentWidth) * -1, targetX);
    ...
    var onControlsClick = function(e) {
      ...
      currentSection = Math.abs(Math.round(currentX / contentWidth));
